		...
		<% if(request.getParameter("totalOrder") != null && !request.getParameter("totalOrder").equals("")) {
			try {
				if(Integer.parseInt(request.getParameter("totalOrder")) <= 0 ) {
					...
					int quantity = 0;		
					//update log table
					PreparedStatement shoppingCartPtst = null, productsCartPtst  = null, logStmt = null;
					...
					try {
						...
						logStmt = con.prepareStatement("insert into log (person_id, product_id, total) values (?, ?, ?)");
						...
						for(int i=0;i<noOfSales;i++) {
							...
							while(productRs.next()) {
								...
								//log each purchase
								logStmt.setInt(1, personId);
								logStmt.setInt(2, productId);
								logStmt.setInt(3, productPrice * quantity);
