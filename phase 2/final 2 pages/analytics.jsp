	...
	<table><tr>
		...
		<td>
			...
			<%	
				...
				Connection conn;
				Statement stmt4;
				PreparedStatement stmt2, stmt3, stmt5, stmt, stmt6;
				...
				try {
					...
					//setting up all statementss
					
					if(rows == null || rows.equals("customers")){
						stmt2 = conn.prepareStatement("select p.person_name, pc.product_id, pr.product_name, sum(pc.quantity * pc.price) As total from " +
								"person p, shopping_cart s, products_in_cart pc, product pr " +
								"where p.id = s.person_id and s.id = pc.cart_id and s.is_purchased = true and p.id = ? " +
	                            "and pr.id = pc.product_id group by p.person_name, pc.product_id, pr.product_name", ResultSet.TYPE_SCROLL_SENSITIVE, 
	                            ResultSet.CONCUR_UPDATABLE);
						if(order == null || order.equals("alphabetical")){
							if(filter == 0){
								stmt = conn.prepareStatement("select person_name, id, sum(total) as total from "+
										"(select p.person_name, p.id, sum(pc.quantity * pc.price) As total from "+
										"person p, shopping_cart s, products_in_cart pc "+
										"where p.id = s.person_id and s.id = pc.cart_id and s.is_purchased = true "+
										"group by p.person_name, p.id union "+
										"select p2.person_name, p2.id, 0 As total from person p2) As temp "+
										"group by person_name, id "+
										"order by person_name limit 20 offset ?", ResultSet.TYPE_SCROLL_SENSITIVE, 
			                            ResultSet.CONCUR_UPDATABLE);
								stmt6 = conn.prepareStatement("select person_name, id, sum(total) as total from "+
										"(select p.person_name, p.id, sum(pc.quantity * pc.price) As total from "+
										"person p, shopping_cart s, products_in_cart pc "+
										"where p.id = s.person_id and s.id = pc.cart_id and s.is_purchased = true "+
										"group by p.person_name, p.id union "+
										"select p2.person_name, p2.id, 0 As total from person p2) As temp "+
										"group by person_name, id "+
										"order by person_name", ResultSet.TYPE_SCROLL_SENSITIVE, 
			                            ResultSet.CONCUR_UPDATABLE);
							}
							else{
								stmt = conn.prepareStatement("select person_name, id, sum(total) as total from "+
										...
										"person p, shopping_cart s, products_in_cart pc, product pr "+
										"where p.id = s.person_id and s.id = pc.cart_id and s.is_purchased = true and pr.id = pc.product_id "+
							...
							if(filter == 0){
								stmt = conn.prepareStatement("select person_name, id, sum(total) as total from "+
										...
										"select p2.person_name, p2.id, 0 As total from person p2) As temp "+
										"group by person_name, id "+
										"order by total desc limit 20 offset ?", ResultSet.TYPE_SCROLL_SENSITIVE, 
			                            ResultSet.CONCUR_UPDATABLE);
								stmt6 = conn.prepareStatement("select person_name, id, sum(total) as total from "+
										"(select p.person_name, p.id, sum(pc.quantity * pc.price) As total from "+
										"person p, shopping_cart s, products_in_cart pc "+
										...
										"select p2.person_name, p2.id, 0 As total from person p2) As temp "+
										"group by person_name, id "+
										"order by total desc", ResultSet.TYPE_SCROLL_SENSITIVE, 
			                            ResultSet.CONCUR_UPDATABLE);
							}
							else{
								stmt = conn.prepareStatement("select person_name, id, sum(total) as total from "+
						...
						stmt2 = conn.prepareStatement("select s.state_name, pc.product_id, pr.product_name, sum(pc.quantity * pc.price) As total from " +
								...
								"and s.id = ? and pr.id = pc.product_id group by s.state_name, pc.product_id, pr.product_name", ResultSet.TYPE_SCROLL_SENSITIVE, 
		                        ResultSet.CONCUR_UPDATABLE);
						if(order == null || order.equals("alphabetical")){
							if(filter == 0){
								stmt = conn.prepareStatement("select state_name, id, sum(total) As total from " +
										...
										"order by state_name limit 20 offset ?", ResultSet.TYPE_SCROLL_SENSITIVE, 
			                            ResultSet.CONCUR_UPDATABLE);
								stmt6 = conn.prepareStatement("select state_name, id, sum(total) As total from " +
										"(select s.state_name, s.id, sum(pc.quantity * pc.price) As total from "+
										"state s, person p, shopping_cart sc, products_in_cart pc "+
										"where s.id = p.state_id and p.id = sc.person_id and sc.id = pc.cart_id and sc.is_purchased = true "+
										"group by s.state_name, s.id union "+
										"select s2.state_name, s2.id, 0 As total from state s2) As temp "+
										"group by state_name, id "+
										"order by state_name", ResultSet.TYPE_SCROLL_SENSITIVE, 
			                            ResultSet.CONCUR_UPDATABLE);
							}
							else{
								stmt = conn.prepareStatement("select state_name, id, sum(total) as total from "+
										...
										"state s, person p, shopping_cart sc, products_in_cart pc, product pr "+
										"where s.id = p.state_id and p.id = sc.person_id and sc.id = pc.cart_id and sc.is_purchased = true and pr.id = pc.product_id "+
					...
					if(order == null || order.equals("alphabetical")){
						if(filter == 0){
							stmt3 = conn.prepareStatement("select product_name, id, sum(total) as total from "+
									...
									"order by product_name limit 10 offset ?", ResultSet.TYPE_SCROLL_SENSITIVE, 
		                            ResultSet.CONCUR_UPDATABLE);
							stmt5 = conn.prepareStatement("select product_name, id, sum(total) as total from "+
									"(select pr.product_name, pr.id, 0 as total from product pr union "+
									"select pr.product_name, pr.id, sum(pc.quantity * pc.price) as total from "+
									"product pr, products_in_cart pc, shopping_cart sc "+
									"where sc.is_purchased = true and sc.id = pc.cart_id and pr.id = pc.product_id "+
									"group by pr.product_name, pr.id) as temp "+
									"group by product_name, id "+
									"order by product_name", ResultSet.TYPE_SCROLL_SENSITIVE, 
		                            ResultSet.CONCUR_UPDATABLE);
						}
						else{
							stmt3 = conn.prepareStatement("select product_name, id, sum(total) as total from "+
						...
						if(filter == 0){
							stmt3 = conn.prepareStatement("select product_name, id, sum(total) as total from "+
									...
									"group by pr.product_name, pr.id) as temp "+
									"group by product_name, id "+
									"order by total desc limit 10 offset ?", ResultSet.TYPE_SCROLL_SENSITIVE, 
		                            ResultSet.CONCUR_UPDATABLE);
							stmt5 = conn.prepareStatement("select product_name, id, sum(total) as total from "+
									"(select pr.product_name, pr.id, 0 as total from product pr union "+
									"select pr.product_name, pr.id, sum(pc.quantity * pc.price) as total from "+
